...
commander
    ...
    .parse(process.argv);
export const translate = async (inputDir = '.', cacheDir = '.json-autotranslate-cache', sourceLang = 'en', targetLangs, deleteUnusedStrings = false, fileType = 'auto', dirStructure = 'default', fixInconsistencies = false, service = 'google-translate', matcher = 'icu', decodeEscapes = false, config) => {
    ...
    console.log(chalk `Found {green.bold ${String(targetLanguages.length)}} target language(s):`);
    ...
    for (const file of templateFiles.filter((f) => f.type === 'natural')) {
        ...
        if (inconsistentKeys.length > 0) {
            ...
            console.log(chalk `├── {yellow.bold ${file.name} contains} {red.bold ${String(inconsistentKeys.length)}} {yellow.bold inconsistent key(s)}`);
        }
    }
    if (inconsistentFiles.length > 0) {
        console.log(chalk `└── {yellow.bold Found key-value inconsistencies in} {red.bold ${String(inconsistentFiles.length)}} {yellow.bold file(s).}`);
        console.log();
        if (fixInconsistencies) {
            ...
            console.log(chalk `Please either fix these inconsistencies manually or supply the {green.bold -f} flag to automatically fix them.`);
        }
    }
    else {
    ...
    for (const file of templateFiles.filter((f) => f.type === 'key-based')) {
        ...
        if (invalidKeys.length > 0) {
            ...
            console.log(chalk `├── {yellow.bold ${file.name} contains} {red.bold ${String(invalidKeys.length)}} {yellow.bold invalid key(s)}`);
        }
    }
    if (invalidFiles.length) {
        console.log(chalk `└── {yellow.bold Found invalid keys in} {red.bold ${String(invalidFiles.length)}} {yellow.bold file(s).}`);
        console.log();
        console.log(chalk `It looks like you're trying to use the key-based mode on natural-language-style JSON files.`);
        console.log(chalk `Please make sure that your keys don't contain periods (.) or remove the {green.bold --type} / {green.bold -t} option.`);
        console.log();
        process.exit(1);
    }
    else {
    ...
    for (const language of targetLanguages) {
        if (!translationService.supportsLanguage(language)) {
            console.log(chalk `🙈 {yellow.bold ${translationService.name} doesn't support} {red.bold ${language}}{yellow.bold . Skipping this language.}`);
            console.log();
            continue;
        }
        console.log(chalk `💬 Translating strings from {green.bold ${sourceLang}} to {green.bold ${language}}...`);
        const translateContent = createTranslator(translationService, service, sourceLang, language, cacheDir, workingDir, dirStructure, deleteUnusedStrings);
        switch (dirStructure) {
            case 'default':
                const existingFiles = loadTranslations(evaluateFilePath(workingDir, dirStructure, language), fileType);
                if (deleteUnusedStrings) {
                    ...
                    for (const file of deletableFiles) {
                        console.log(chalk `├── {red.bold ${file.name} is no longer used and will be deleted.}`);
    ...
    if (service !== 'dry-run') {
        ...
        await new Promise((res, rej) => ncp(evaluateFilePath(workingDir, dirStructure, sourceLang), evaluateFilePath(resolvedCacheDir, dirStructure, sourceLang), (err) => (err ? rej() : res(null))));
...
translate(commander.input, commander.cacheDir, commander.sourceLanguage, commander.deleteUnusedStrings, commander.type, commander.directoryStructure, commander.fixInconsistencies, commander.service, commander.matcher, commander.decodeEscapes, commander.config).catch((e) => {
    console.log();
...
function createTranslator(translationService, service, sourceLang, targetLang, cacheDir, workingDir, dirStructure, deleteUnusedStrings) {
    return async (sourceFile, destinationFile) => {
        const cachePath = path.resolve(evaluateFilePath(cacheDir, dirStructure, sourceLang), sourceFile ? sourceFile.name : '');
        let cacheDiff = [];
        if (fs.existsSync(cachePath) && !fs.statSync(cachePath).isDirectory()) {
            const cachedFile = flatten.convert(JSON.parse(fs.readFileSync(cachePath).toString().trim()));
            ...
            process.stdout.write(chalk ` ({green.bold ${changedItems}} changes from cache)`);
        }
        ...
        if (service !== 'dry-run') {
            ...
            fs.writeFileSync(path.resolve(evaluateFilePath(workingDir, dirStructure, targetLang), destinationFile?.name ?? sourceFile.name), newContent);
        ...
        console.log(deleteUnusedStrings && unusedStrings.length > 0
            ? chalk ` ({green.bold +${String(translatedStrings.length)}}/{red.bold -${String(unusedStrings.length)}})`
