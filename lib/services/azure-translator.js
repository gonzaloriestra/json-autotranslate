...
export class AzureTranslator {
    ...
    async translateBatch(batch, from, to) {
        ...
        const response = await fetch(`${TRANSLATE_ENDPOINT}&from=${from}&to=${to}&textType=html`, {
            method: 'POST',
            headers,
        ...
        return data.map((res, i) => {
            const translated = reInsertInterpolations(res.translations[0].text, toTranslate[i].replacements);
            return {
